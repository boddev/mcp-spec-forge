...
function loadEvalSet(filePath) {
  ...
  if (lower.endsWith(".jsonl")) {
    ...
    rows = Array.isArray(parsed) ? parsed : parsed.questions ?? parsed.evalSet ?? parsed.items ?? [];
  } else {
    rows = parse(raw, {
      columns: true,
...
function tokenize(text) {
  return text.replace(/([a-z0-9])([A-Z])/g, "$1 $2").replace(/[_\-/{}.]/g, " ").toLowerCase().replace(/[^a-z0-9 ]/g, " ").split(/\s+/).map(singularize).filter((t) => t.length > 1 && !STOPWORDS.has(t));
}
...
// src/ingest/openapi.ts
var HTTP_METHODS = ["get", "put", "post", "delete", "patch", "head", "options"];
async function loadOpenApi(source) {
  ...
  if (/^https?:\/\//i.test(source)) {
    ...
    raw = (0, import_node_fs2.readFileSync)(source, "utf8");
  }
...
function parseDoc(raw) {
  ...
  return index_vite_proxy_tmp_default.load(raw);
}
function normalizeOpenApi(doc) {
  ...
  for (const [path, pathItemRaw] of Object.entries(paths)) {
    ...
    const sharedParams = (pathItem.parameters ?? []).map((p) => deref(p, doc));
...
function buildCard(method, path, op, sharedParams, doc, globalSecurity) {
  const params = [...sharedParams, ...(op.parameters ?? []).map((p) => deref(p, doc))];
...
function extractParams(params, op, doc) {
  const out = [];
  ...
  if (bodySchema?.properties) {
    ...
    for (const [name, schema] of Object.entries(bodySchema.properties)) {
      out.push({
        ...
        type: deref(schema, doc)?.type
      });
    }
  }
  return out;
}
function extractResponseFields(op, doc) {
  const responses = op.responses ?? {};
  const successKey = Object.keys(responses).find((k) => /^2\d\d$/.test(k)) ?? (responses.default ? "default" : void 0);
...
function collectFields(schema, prefix, out, doc, depth) {
...
function deref(node, doc, seen = /* @__PURE__ */ new Set()) {
  ...
  if (typeof node.$ref === "string") {
    ...
    const target = ref.slice(2).split("/").reduce((acc, key) => acc ? acc[decodeRefToken(key)] : void 0, doc);
...
function securityNames(security, doc) {
...
function classifyVerb(method, path, summary, opId) {
...
function extractEntities(path, summary, opId) {
...
// src/ingest/htmlDocs.ts
var import_node_fs3 = require("node:fs");
var METHOD_PATH_RE = /\b(GET|POST|PUT|PATCH|DELETE)\s+(\/[A-Za-z0-9_\-/{}.:]+)/g;
async function loadHtmlDocs(source) {
  let html;
  if (/^https?:\/\//i.test(source)) {
    const res = await fetch(source);
    ...
    html = (0, import_node_fs3.readFileSync)(source, "utf8");
  }
  return extractFromHtml(html);
}
function extractFromHtml(html) {
  const stripped = html.replace(/<(script|style|nav|footer)\b[^>]*>[\s\S]*?<\/\1>/gi, " ").replace(/<!--[\s\S]*?-->/g, " ");
  ...
  while ((match = METHOD_PATH_RE.exec(text)) !== null) {
    ...
    const summary = text.slice(ctxStart, ctxEnd).replace(/\s+/g, " ").trim().slice(0, 140);
...
// src/analyze/questionAnalyzer.ts
var DETAIL_SIGNALS = [
  "each",
  "every",
  "all",
...
function analyzeOne(q) {
  ...
  const needsDetailExpansion = DETAIL_SIGNALS.some(
    (s) => q.prompt.toLowerCase().includes(s)
  );
  return {
    id: q.id,
    prompt: q.prompt,
...
function groupIntoTools(plans, endpoints, opts = {}) {
  ...
  const questionEndpoints = /* @__PURE__ */ new Map();
...
function selectQuestionEndpoints(plan) {
...
function shareDominantEntity(a, b) {
  const ea = new Set(a.entities.slice(0, 3));
  return b.entities.slice(0, 3).some((e) => ea.has(e));
}
function buildTool(cards, coveredQuestions, isChain, usedNames, limits) {
  ...
  return {
    ...
    outputSchema,
    underlyingCalls,
    pagination: listLike.length ? { strategy: "auto_paginate_until_limit", maxPages: 5 } : void 0,
    rateLimiting: {
...
function buildCalls(ordered, isChain) {
  const calls = [];
  let firstListId;
...
function buildInputSchema(ordered, isChain) {
  const properties = {};
  const required = [];
  ...
  return { type: "object", required: required.length ? required : void 0, properties };
}
function buildOutputSchema(ordered) {
...
function uniqueTopFields(cards) {
  const out = [];
...
function dominantEntities(cards) {
  ...
  return [...freq.entries()].sort((a, b) => b[1] - a[1]).map(([e]) => e).slice(0, 4);
}
function toolName(cards, entities, isChain) {
...
function toolDescription(cards, entities, covered, isChain) {
  const ents = entities.join(", ") || "records";
  const base = `Task-oriented tool for ${ents}. Wraps ${cards.length} API endpoint(s)`;
  const chain = isChain ? ", collapsing a list-then-detail (N+1) pattern into a single call" : "";
...
// src/generate/specBuilder.ts
function buildDesign(args) {
  ...
  return {
    server: {
      ...
      description: args.description ?? `MCP server designed from an eval set of ${args.questions.length} questions over ${args.endpoints.length} documented API endpoint(s).`,
      version: "0.1.0",
      transport: "stdio"
    },
    auth,
...
// src/generate/emitYaml.ts
function emitYaml(design) {
  const clean = JSON.parse(JSON.stringify(design));
  const header = "# MCP server design \u2014 generated by mcp-spec-forge\n# Canonical, machine-readable spec. See mcp-design.md for rationale.\n";
  return header + index_vite_proxy_tmp_default.dump(clean, { lineWidth: 100, noRefs: true, sortKeys: false });
}

// src/generate/emitMarkdown.ts
function emitMarkdown(design, report) {
  const lines = [];
  const d = design;
...
// src/generate/scaffold.ts
function scaffoldServer(design) {
  const files = {};
  ...
  files["README.md"] = `# ${design.server.name}

${design.server.description}

Scaffold generated by mcp-spec-forge. Implement each tool handler in \`src/tools/\` by calling the documented underlying API endpoints. Auth: \`${design.auth.type}\`${design.auth.envVar ? ` via env \`${design.auth.envVar}\`` : ""}.
`;
  ...
  files["src/tools/index.ts"] = design.tools.map((t) => `export { ${t.name} } from './${t.name}.js';`).join("\n") + "\n";
  return files;
}
function serverEntry(design) {
  const imports = design.tools.map((t) => t.name).join(", ");
  const registrations = design.tools.map(
...
function toolModule(tool) {
  const calls = tool.underlyingCalls.map((c) => `  //  - ${c.endpoint} :: ${c.purpose}${c.foreach ? ` (foreach ${c.foreach})` : ""}`).join("\n");
...
function validateDesign(design, plans, endpoints) {
  const endpointIds = new Set(endpoints.map((e) => e.id));
  const unknownEndpointRefs = [];
  for (const tool of design.tools) {
  ...
  const toolByEndpoint = /* @__PURE__ */ new Map();
  for (const tool of design.tools) {
    for (const call of tool.underlyingCalls) {
  ...
  const coverage = plans.map((plan) => {
    ...
    const tool = toolByEndpoint.get(primary.endpoint.id) ?? design.tools.find((t) => t.evalCoverage.includes(plan.question.id));
    const notes = [];
  ...
  const godTools = design.tools.filter((t) => t.reviewFlags.some((f) => f.startsWith("broad-scope") || f.startsWith("many-endpoints"))).map((t) => t.name);
  return {
    coverage,
    uncovered,
    orphanTools,
...
function writeResult(result, outDir, withScaffold) {
  (0, import_node_fs4.mkdirSync)(outDir, { recursive: true });
  const written = [];
  const write = (rel, content) => {
    const full = (0, import_node_path.join)(outDir, rel);
    (0, import_node_fs4.mkdirSync)((0, import_node_path.dirname)(full), { recursive: true });
    (0, import_node_fs4.writeFileSync)(full, content, "utf8");
    written.push(full);
  };
  ...
  if (withScaffold) {
    for (const [rel, content] of Object.entries(result.scaffold)) {
      write((0, import_node_path.join)("server-scaffold", rel), content);
    }
  }
  return written;
}
...
program2.name("mcp-spec-forge").description("Forge an MCP server design spec from an eval set + documented APIs.").version("0.1.0");
program2.command("generate").description("Ingest an eval set and API docs, then emit an MCP design spec.").requiredOption("-e, --eval <path>", "eval set file (.csv, .json, .jsonl)").option("-o, --openapi <source...>", "OpenAPI doc(s): file path or URL", []).option("-d, --html <source...>", "HTML API doc(s): file path or URL (best-effort)", []).option("-n, --name <name>", "MCP server name", "forged-mcp-server").option("--description <text>", "server description").option("-O, --out <dir>", "output directory", "./out").option("--scaffold", "also emit a runnable TypeScript MCP server scaffold", false).option("--llm", "enable LLM grouping refinement (requires provider)", false).action(async (opts) => {
  try {
