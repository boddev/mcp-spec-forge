...
function scaffoldServer(design, transport = "both") {
  const files = {};
  ...
  files["src/tools/index.ts"] = design.tools.map((t) => `import { ${t.name} } from './${t.name}.js';`).join("\n") + `
...
function packageJson(design, wantStdio, wantHttp) {
  const scripts = {};
  ...
  scripts.build = "tsc";
  const dependencies = { "@modelcontextprotocol/sdk": "^1.0.0" };
...
function buildServerModule(design) {
...
function apiClient(design) {
  ...
  const authHeader = design.auth.type === "none" ? "" : design.auth.type === "api_key" ? `  if (TOKEN) headers['x-api-key'] = TOKEN;
...
var ORCHESTRATE = `import { apiCall } from './client.js';
...
function toStep(call) {
  ...
  const step = { id: call.id, method, path, pathParams };
  if (call.purpose) step.purpose = call.purpose;
  if (call.foreach) {
    const dot = call.foreach.lastIndexOf(".");
    step.foreach = dot > -1 ? { sourceId: call.foreach.slice(0, dot), hint: call.foreach.slice(dot + 1) } : { sourceId: call.foreach, hint: "items" };
  }
  if (call.condition) {
...
function readme(design, wantStdio, wantHttp) {
  const lines = [];
  ...
  lines.push(
    ...
    "```bash",
    "npm install",
    "cp .env.example .env   # set API_BASE_URL" + (design.auth.type !== "none" && design.auth.envVar ? ` and ${design.auth.envVar}` : ""),
    "```",
    "",
